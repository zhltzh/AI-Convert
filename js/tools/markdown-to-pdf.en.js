...
exportButton.addEventListener('click', async () => { exportButton.disabled = true; exportButton.textContent = 'Creating PDF…'; try { await exportPdf(preview.innerHTML, 'aixuno-document.pdf'); status.textContent = 'Your PDF is downloading.'; } catch { status.textContent = 'Unable to create the PDF. Check your connection and try again.'; } finally { exportButton.disabled = false; exportButton.innerHTML = 'Download PDF <span aria-hidden="true">→</span>'; } });
updatePreview();
