...
export function renderIntlHeader() {
  ...
  const languageLinks = languageOrder.map(code => `<a href="${languageHref(code)}" hreflang="${code}" lang="${code}" data-locale="${code}">${labels[code]}</a>`).join('');
...
export function renderIntlFooter() {
  ...
  const prefix = '/';
  const words = {
    en:['About','Contact','Privacy','Terms','Disclaimer','Changelog','Your content stays in your browser.','Image tools','AI cutout — Coming soon','PDF tools — Coming soon'],
    es:['Acerca de','Contacto','Privacidad','Términos','Aviso legal','Cambios','Tu contenido permanece en tu navegador.','Herramientas de imagen','Recorte con IA — Próximamente','Herramientas PDF — Próximamente'],
    de:['Über uns','Kontakt','Datenschutz','Nutzungsbedingungen','Haftungsausschluss','Änderungen','Deine Inhalte bleiben im Browser.','Bildwerkzeuge','KI-Freisteller — Demnächst','PDF-Werkzeuge — Demnächst'],
    ja:['概要','お問い合わせ','プライバシー','利用規約','免責事項','更新履歴','コンテンツはブラウザ内に保持されます。','画像変換','AI切り抜き — 公開予定','PDFツール — 公開予定'],
